...
function showView(view){ document.querySelectorAll('[data-view-panel]').forEach(p=>p.hidden=p.dataset.viewPanel!==view); document.querySelectorAll('.nav-item').forEach(li=>li.classList.toggle('active',li.querySelector(`[data-view="${view}"]`)!==null)); const titles={general:'Resumen general',sucursales:'Sucursales',usuarios:'Usuarios',inventario:'Inventario',ventas:'Ventas'}; $('#topbarTitle').textContent=titles[view]; location.hash=view; $('#appShell').classList.remove('mobile-open'); if(view==='sucursales') loadSucursales(); if(view==='usuarios') loadUsuarios(); if(view==='inventario') loadInventario(); if(view==='ventas') loadVentas(); }
...
$('#addSucursal').addEventListener('click',()=>openModal('sucursal')); $('#addUsuario').addEventListener('click',()=>openModal('usuario')); $('#closeModal').addEventListener('click',closeModal); $('#entityModal').addEventListener('click',e=>{if(e.target===e.currentTarget||e.target.id==='cancelModal')closeModal();});
$('#entityForm').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget, type=f.dataset.type, action=`${f.dataset.editing?'update':'create'}_${type==='usuario'?'gerente':'sucursal'}`;try{const d=await request(action,{method:'POST',body:new FormData(f)});toast(d.msg);closeModal();type==='sucursal'?loadSucursales():loadUsuarios();loadBranches();}catch(err){toast(err.message,'error');}});
document.addEventListener('click', async e=>{const editS=e.target.closest('[data-edit-sucursal]'),editU=e.target.closest('[data-edit-usuario]'),delS=e.target.closest('[data-delete-sucursal]'),delU=e.target.closest('[data-delete-usuario]'); if(editS)openModal('sucursal',JSON.parse(editS.dataset.editSucursal)); if(editU)openModal('usuario',JSON.parse(editU.dataset.editUsuario)); const del=delS||delU;if(del&&confirm('¿Eliminar este registro? Esta acción no se puede deshacer.')){try{const fd=new FormData();fd.append('id',delS?delS.dataset.deleteSucursal:delU.dataset.deleteUsuario);const d=await request(delS?'delete_sucursal':'delete_gerente',{method:'POST',body:fd});toast(d.msg);delS?loadSucursales():loadUsuarios();loadBranches();}catch(err){toast(err.message,'error');}}});
['#inventorySearch','#inventoryBranch'].forEach(s=>$(s).addEventListener('input',loadInventario)); ['#salesBranch','#salesFrom','#salesTo'].forEach(s=>$(s).addEventListener('change',loadVentas));
...
window.addEventListener('DOMContentLoaded',()=>{const palette=['#936545','#547c5c','#648ba6','#b3832e','#a66d7e']; const labels=initial.labels||[]; new Chart($('#salesChart'),{type:'line',data:{labels,datasets:[{label:'Ingresos',data:initial.ingresos||[],borderColor:'#6c4630',backgroundColor:'rgba(147,101,69,.14)',fill:true,tension:.38,pointRadius:3}]},options:{responsive:true,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{callback:v=>'$'+v}},x:{grid:{display:false}}}}}); new Chart($('#paymentChart'),{type:'doughnut',data:{labels:(initial.payments||[]).map(x=>x.metodo),datasets:[{data:(initial.payments||[]).map(x=>x.total_ventas),backgroundColor:palette,borderWidth:0}]},options:{cutout:'72%',plugins:{legend:{position:'bottom',labels:{boxWidth:10,padding:14}}}}}); loadLowStock();loadBranches(); const view=location.hash.replace('#','');if(['sucursales','usuarios','inventario','ventas'].includes(view))showView(view);});
</script>
</body>
</html>
